  ...
  return (
    <div className="flex flex-col h-full min-h-0 overflow-hidden">
      ...
      <div className="flex items-center gap-2 sm:gap-3 flex-wrap px-4 sm:px-5 pt-4 pb-2">
        ...
        <button className="hidden sm:inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-md text-xs font-medium border border-[#222b39] bg-[#161c27] text-[#aeb8c8] hover:bg-[#1c2430] transition-colors">
      ...
      {/* Content */}
      <div className="flex-1 flex flex-col md:flex-row gap-3.5 px-4 sm:px-5 pb-5 min-h-0">
        {/* Alert Timeline */}
        <div className="flex-1 rounded-md border border-[#222b39] bg-[#11161f] flex flex-col min-h-0">
          ...
          <div className="flex-1 overflow-auto min-h-0">
            {alerts.map((a, i) => (
              <button
                key={i}
                onClick={() => { setSelectedIdx(i); setShowDetail(true); }}
              ...
              >
                ...
                <ChevronRight size={13} className="text-[#4a5567] flex-shrink-0 hidden md:block" />
              </button>
            ))}
          </div>
        </div>

        {/* Alert Detail - desktop: side panel, mobile: overlay */}
        <div
          className="hidden md:flex rounded-md border border-[#222b39] bg-[#11161f] flex-col w-[380px] min-h-0 flex-shrink-0"
        >
          <AlertDetailContent alert={alert} severityColor={severityColor} sevLabel={sevLabel} telemetryData={telemetryData} />
        </div>

        {/* Mobile detail overlay */}
        {showDetail && (
          <div
            className="md:hidden fixed inset-0 z-50 animate-fade-in-overlay"
            onClick={() => setShowDetail(false)}
          >
            <div className="absolute inset-0 bg-black/60 backdrop-blur-sm" />
            <div
              className="absolute inset-x-2 bottom-2 rounded-xl border border-[#222b39] bg-[#11161f] overflow-hidden max-h-[80vh] animate-slide-up"
              onClick={(e) => e.stopPropagation()}
            >
              {/* Drag handle */}
              <div className="flex justify-center pt-2 pb-1">
                <div className="w-10 h-1 rounded-full" style={{ backgroundColor: '#2c3647' }} />
              </div>
              <AlertDetailContent alert={alert} severityColor={severityColor} sevLabel={sevLabel} telemetryData={telemetryData} />
            </div>
          </div>
...
// ─── Alert Detail Content (shared between desktop & mobile) ────────────────

function AlertDetailContent({ alert, severityColor, sevLabel, telemetryData }: {
      ...
      <div className="flex-1 overflow-auto min-h-0 p-4">
        <div className="flex flex-col gap-3">
          ...
          <div className="flex gap-2">
            <button className="flex-1 inline-flex items-center justify-center gap-1.5 px-3 py-2.5 rounded-md text-xs font-semibold text-[#1a0f00]" style={{
              background: 'linear-gradient(180deg, #f4a417 0%, #d98d0a 100%)',
              border: '1px solid #a06b08',
            }}>
              <Check size={12} /> Apply Fix
            </button>
            <button className="inline-flex items-center gap-1.5 px-2.5 py-2.5 rounded-md text-xs font-medium border border-[#222b39] bg-transparent text-[#aeb8c8] hover:bg-[#161c27] transition-colors">
              <X size={12} /> Snooze
            </button>
          </div>
